...
def getKey(keyLength):
    ...
    return "".join(rnd.choice(letters) for i in range(keyLength))

def main():
    key = getKey(rnd.randint(3, 9))
    print(key)
    encrypted = encrypt(input("Hva er meldingen? "), key)
    
    print("Krypto:", encrypted)
    print("Melding:", decrypt(encrypted, key))
