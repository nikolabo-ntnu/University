...
def findSubstringIndexes(find, string):
    return [i for i in range(len(string) - len(find) + 1) if string[i:i+len(find)].lower() == find.lower()]


str2 = "Never let you go let me go. Never let me go ooo"
print(findSubstringIndexes("Never", str2))
...
def replaceSubstring(find, string, replace):
    indexes = findSubstringIndexes(find, string)
    result = ""
    for i in range(len(indexes)):
        if i == 0:
            result += string[:indexes[i]]
        else:
            result += string[indexes[i-1] + len(find):indexes[i]]
            
        result += replace
        
        if i == len(indexes) - 1:
            result += string[indexes[i]+len(find):]
...
print(replaceSubstring(str1, str2, str3))
